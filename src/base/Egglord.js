...
class Egglord extends Client {
	constructor() {
		super({
			...
			intents: ['GUILDS', 'GUILD_MEMBERS', 'GUILD_BANS', 'GUILD_EMOJIS_AND_STICKERS', 'GUILD_MESSAGES',
				'GUILD_MESSAGE_REACTIONS', 'DIRECT_MESSAGES', 'GUILD_VOICE_STATES', 'GUILD_INVITES', 'GUILD_SCHEDULED_EVENTS','GUILD_PRESENCES'],
			presence: {
				status: 'online',
				activities: [{
					name: 'CSJGAMING.com V Delta',
					type: 'LISTENING',
	...
	*/
	SetActivity(type, array = []) {
		...
		try {
			let j = 0;
			setInterval(() => this.user.setActivity(`${this.Activity[j++ % this.Activity.length]}`, { type: type, url: 'https://www.twitch.tv/csj90' }), 10000);
		} catch (e) {
			console.log(e);
		}
	}
