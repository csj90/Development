...
class UserInfo extends Command {
  ...
  */
	...
	*/
	...
	const LegacyStaff = ["89695287366414336","129172043987025921","106048100002439168","169896575336710144"];
	const Contributors = ["263895063573037057"];
	const Developers = ["105419972309741568"];
	const Friends = ["489133814765846538"];
	...
	if (Friends.includes(id)) teamAcks.push("Friends");
    // rest of your if statements go here
 
       return teamAcks;
	};
		//if(getAcks(ID).length >= 0)
	...
	createEmbed(bot, guild, member) {
		...
		return new Embed(bot, guild)
			...
			.addFields(
				...
				{ name: if (getAcks(id).length > 0) ("Staff Acknowledgements", getAcks(id), inline: true },
